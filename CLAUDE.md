...
Personal engineering portfolio for Oscar Stomberg, Robotics Engineering student at WPI. See [README.md](README.md) for stack and commands. **Note:** the README's project structure diagram is partially stale — `src/components/mdx/` (YouTube, GreetingDemo, LiveGreeting, GreetingFallback) is not reflected. CLAUDE.md is the authoritative reference for component paths. See [ROADMAP.md](ROADMAP.md) for in-flight follow-ups and known improvements.
...
1. Drop a `.md` or `.mdx` file in `src/content/projects/` with the frontmatter schema below. Use `.mdx` when the body imports components (e.g. `YouTube`, or `Figure` for any in-article image — see step 3a).
2. Add a thumbnail at `src/assets/<slug>/thumbnail.{png,jpg,jpeg,webp,avif}` (resolved by `findThumbnailImage()` / `findThumbnailPath()` in `src/lib/thumbnails.ts` — no frontmatter field). If absent, the thumbnail renders as a diagonal-hatch placeholder.
3. (Optional) Drop gallery images at `src/assets/<slug>/*.{jpg,jpeg,png,webp,avif}`. They're discovered automatically by `loadGalleryItems()` and appear on `/gallery`. They render through Astro's `<Image>`, so Sharp generates responsive srcsets at build time.
   - **3a. In-article images** live in the same `src/assets/<slug>/` folder and are referenced from the MDX body with `<Figure src="<filename>" alt="…" caption="…" />` (import `Figure` from `../../components/mdx/Figure.astro`) — **not** raw markdown `![]()`. The same file can serve double duty as both a gallery item and a `<Figure>` source. Use a bare filename; qualify as `"<slug>/<file>"` only if the basename collides with another project's image. To show several images side by side at matching height, wrap the `<Figure>`s in `<FigureRow>` (import from `../../components/mdx/FigureRow.astro`).
...
### Adding a demo
1. Set `demo: true` in the project's frontmatter.
2. In the MDX body, wrap whichever embed is "the demo" (a `<YouTube>`, or an interactive island like `LiveGreeting`) in `<DemoAnchor>` (import from `../../components/mdx/DemoAnchor.astro`).
3. That's it — `ProjectThumbnail` picks up `demo` wherever it renders that project (home, `/projects`, Related cards) and shows a corner badge linking to `/projects/<slug>#demo`. No manual wiring needed per callsite.
...
A separate `greeting-proxy` Worker (standalone repo at `~/Dropbox/Projects/027 Greeting Demo/`, `wrangler deploy`) runs at `oscar.stomberg.us/api/greeting/*`, sitting in front of the Astro Worker for that path. It accepts authenticated uploads from the home server and serves cached data to visitors.

**Push model.** A local trigger script (Ansible-managed, `roles/greeting_scheduler/` in the selfhosting repo) runs on cron, generates a greeting via the Flask API on the LAN, then POSTs the results to `POST /api/greeting/upload` as multipart form-data (metadata JSON + audio WAV + cover JPEG) with `Authorization: Bearer $UPLOAD_SECRET`. The Worker validates the token, writes metadata to KV and assets to R2 in parallel, and returns 200. No Cloudflare Tunnel, no cron on the Worker, no Go file server -- the home server pushes, never gets pulled from. `UPLOAD_SECRET` is a Wrangler secret.
...
The greeting generator source lives at `~/Dropbox/Projects/021 Daily Greeting/` (Flask app, Docker image `potatorocket/daily-greeting`). Both directories are configured as `additionalDirectories` in `.claude/settings.local.json` so Claude can read them directly.
...
demo: false                    # true → thumbnail gets a corner badge linking to /projects/<slug>#demo (see Adding a demo)
...
- `FeaturedCard.astro` — homepage tile. Outer element is a `relative` `<div>` (not an `<a>`): the title is the real navigational anchor, stretched over the whole card via `after:absolute after:inset-0 after:content-['']` (the "stretched-link" pattern), which is what lets the demo badge (see `ProjectThumbnail.astro` below) coexist as a second, independently clickable anchor without nesting `<a>`s. Width-agnostic by default; the home page sizes it via `class="w-88 max-w-full"` on each instance inside a `flex flex-wrap justify-center` container, which gives fixed-width cards with an incomplete last row centered (CSS Grid auto-fill can't do the per-row centering). **Trailer-style content**: thumbnail, italic-muted summary lead, date. No tags — tags live on `/projects` and the article hero.
- `RelatedCard.astro` — bordered card variant used in the "Related" section at the bottom of a project detail page. Thinner than `FeaturedCard`, includes the date label. Same stretched-link structure as `FeaturedCard` (title anchor stretched over the card).
- `ProjectThumbnail.astro` — static thumbnail box (aspect-video, strong border, `relative`). Resolves via `findThumbnailImage(slug)` and renders with `<Image widths={[640,1280]}>` for Sharp srcset; falls back to a diagonal-hatch SVG. Accepts `loading?: "lazy" | "eager"` (default `"lazy"`); pass `loading="eager"` for above-the-fold callsites — `fetchpriority="high"` is set automatically. Accepts optional `demoHref?: string`: when set, renders an absolutely-positioned corner badge (paper-backed `lucide:circle-play` icon, top-right) that links straight there — used for `/projects/<slug>#demo` deep links (see [Adding a demo](#adding-a-demo) below). Used by `FeaturedCard`, `RelatedCard`, and the detail-page hero (the hero omits `demoHref` — no point deep-linking to yourself).
...
- `DemoAnchor.astro` (`src/components/mdx/`) — plain `<div id="demo" class="scroll-mt-24"><slot /></div>` wrapper, no styling of its own. Wrap it around whichever embed is "the demo" in a project's MDX body (a `<YouTube>`, or an interactive island like `LiveGreeting`) so the thumbnail corner badge can deep-link to `/projects/<slug>#demo` (mirrors the `#contact` + `scroll-mt-24` anchor pattern already used by `ContactForm.svelte`). See [Adding a demo](#adding-a-demo).
- `Figure.astro` (`src/components/mdx/`) — `not-prose` article-body image + caption wrapper. Props: `src` (a **filename string**, e.g. `"diagram.png"` — resolved against `src/assets/**` by `findArticleImage()`, **no import needed**; pass `"<slug>/diagram.png"` to disambiguate when a basename isn't unique), `alt`, optional `caption` (rendered as centered italic `figcaption`), optional `maxHeight` (default `"36rem"`, `false` disables), optional `widths` / `sizes` / `loading` / `class` overrides (defaults: `[640,960,1280]`, `(max-width: 720px) 100vw, 720px`, `"lazy"`). Renders the resolved `ImageMetadata` through Astro's `<Image>` (Sharp srcset at build time) with a `border-rule-strong`. **Height cap:** at `sm:` and up, the `<figure>` gets `max-width: maxHeight * aspectRatio` (via the inline `--fig-max-w` var) and `mx-auto`, so tall portraits are capped and centered with the caption narrowing to match; below `sm` images always go full width. Landscape images in the article column rarely hit the default. Throws at build if the filename matches no file or is ambiguous. Use for in-article images that aren't gallery items; e.g. `import Figure from "../../components/mdx/Figure.astro"` then `<Figure src="diagram.png" alt="..." caption="..." />`.
- `FigureRow.astro` (`src/components/mdx/`) -- wraps multiple `<Figure>`s side-by-side at a shared height. Each `Figure` publishes its aspect ratio as the inline `--fig-ar` var; the row gives each child `flex: var(--fig-ar)` (grow = aspect ratio, basis 0), so widths are proportional to aspect ratio and heights match -- the same trick as the gallery's justified rows. Overrides the children's `maxHeight` cap and margins (the row's height falls out of its width). Below `xs` the figures stack full width. Per-image captions hang below each image (`items-start`). Known trade-off: children keep `Figure`'s default `sizes` (720px), so the browser picks a larger srcset candidate than a half-width slot needs; pass a narrower `sizes` on the child `Figure`s if that matters. Usage: `<FigureRow><Figure .../><Figure .../></FigureRow>`.
...
- `ProjectThumbnail.svelte` — used inside the timeline rows so the thumbnail and its `<a>` wrapper participate in the row's reactive layout. Root is a `relative` wrapper `<div>`; the thumbnail image is its own `<a>` with `aria-hidden + tabindex=-1` because the title link below is the canonical, named link for screen readers. Accepts optional `demoHref: string | null`, rendering the same corner badge as the Astro version (via `@iconify/svelte`'s `Icon` instead of `astro-icon`) as a sibling anchor — not nested inside the aria-hidden one.
- `LiveGreeting.svelte` (`src/components/mdx/`) -- `client:load` island on the daily-greeting-generator page. Fetches `/api/greeting` (JSON metadata) from the greeting Worker, then renders cover and audio from `/api/greeting/cover?d=${data.date}` and `/api/greeting/audio?d=${data.date}` -- the date query is the cache-busting key (see [Caching strategy](#greeting-worker-route)). Three states: loading, success, offline. Status dot: green = `lastSynced` is within 24h, yellow = `lastSynced` is >24h ago (`isStale()`), red = fetch failed. Caption always shows `syncedAgo(lastSynced)` (e.g. "synced 3 hours ago", "synced 2 days ago"). In dev, mocks data immediately from `src/data/daily-greeting.json`; `?greeting=offline` exercises the offline state and `?greeting=stale` sets `lastSynced` 48h in the past to trigger the yellow dot. Optional `devCoverSrc` / `devAudioSrc` props let the page pass local fallback URLs so the cover and audio render under `npm run dev` without the Worker. Dispatches `CustomEvent('greeting:live')` on `document` on success so `GreetingFallback` can hide itself.
...
- `src/lib/thumbnails.ts` — thumbnails live at `src/assets/<slug>/thumbnail.{png,jpg,jpeg,webp,avif}`, next to the project's other assets. Exports `findThumbnailImage(slug): ImageMetadata | null` (for `.astro` `<Image>` use) and `findThumbnailPath(slug): string | null` (hashed URL for Svelte props). Uses **two separate globs**: one without `?url` for `ImageMetadata`, one with `{ query: "?url" }` for the URL string. **Gotcha:** `ImageMetadata.src` from a plain glob is a dev-only `/@fs/...` URL — always use the `?url` glob when you need a URL that works in production (e.g. props passed to `client:load` islands, OG image tags).
- `src/lib/projectAssets.ts` — single source of truth for "what counts as a project asset image": exports `projectAssetImages`, an eager glob map (path → `ImageMetadata`) over `src/assets/**/*.{jpg,jpeg,png,webp,avif}` with each project's `thumbnail.*` excluded (so thumbnails never show up in the gallery or resolve as a `<Figure>`; `thumbnail` is a reserved filename in asset folders). **Both** `<Figure>` (via `findArticleImage()`) and the gallery (`loadGalleryItems()`) read this one map so their notion of the asset set can't drift — keep the extension list here authoritative. Also exports `findArticleImage(src)`, which resolves an in-article image from a bare filename or `<slug>/file` path (throws on an ambiguous bare filename).
...
- `src/lib/dailyGreetingAssets.ts` — single source of truth for resolving the daily-greeting-generator's album cover art, which may be re-exported as either png or jpg. Exports `albumCoverUrl` (globs `album.{png,jpg,jpeg}`, throws at build if missing). Both `StaticGreeting.astro` and the project's own MDX body (`devCoverSrc` passed to `LiveGreeting`) import from here — don't hardcode the extension in either place again.
...
- `src/assets/<slug>/` — per-project assets: `thumbnail.<ext>` (the thumbnail) plus gallery/article images (any combination of `.jpg/.jpeg/.png/.webp/.avif`). Discovered by `loadGalleryItems()`.
- `src/assets/thumbnail.png` — site-wide default OG image (top-level, used by `Layout.astro`); not a project thumbnail.
...
In-article images use the `<Figure>` component (see [Components](#astro-components) and [Adding a new project](#adding-a-new-project) step 3a), **not** raw markdown `![]()` — every existing article image was converted to `<Figure>`, which adds the bordered + italic-caption treatment. New body images should follow suit.
